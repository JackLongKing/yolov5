...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        imgs_root = os.path.dirname(self.img_files[0])
        labels_root = imgs_root.replace('images', 'labels')
        print('labels root path: {}, img root: {}, pls check it they right or not.'.format(labels_root, imgs_root))
        ...
        if len(self.img_files) != len(self.label_files):
            ...
            print('original imgs: {}, original labels: {}, valid num: {}'.format(len(self.img_files), len(self.label_files), len(valid_files)))
            # labels is more than images
            self.label_files = [os.path.join(labels_root, i + '.txt') for i in valid_files]
            self.img_files = [os.path.join(imgs_root, i + '.jpg') for i in valid_files]
        self.img_files = sorted(self.img_files)
        self.label_files = sorted(self.label_files)
        print('img num: {}, label num: {}'.format(len(self.img_files), len(self.label_files)))
        print(self.img_files[89])
        print(self.label_files[89])
